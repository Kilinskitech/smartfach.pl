...
export default async function Page({ searchParams }: { searchParams: Promise<{ dalej?: string; plan?: string; typ?: string }> }) {
  ...
  return <AuthForm next={params.dalej?.startsWith("/") ? params.dalej : "/app"} initialPlan={selectedPlan.success ? selectedPlan.data : "pro"} initialAccountType={selectedType.success ? selectedType.data : "discover"} />;
}
