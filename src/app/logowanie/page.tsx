...
export default async function Page({ searchParams }: { searchParams: Promise<{ dalej?: string; plan?: string; anulowano?: string; blad?: string }> }) {
  ...
  return <AuthForm loginFirst={Boolean(params.dalej) || !params.plan} next={params.dalej?.startsWith("/") ? params.dalej : "/app"} initialPlan={selectedPlan.success ? selectedPlan.data : "pro"} checkoutCanceled={params.anulowano === "1"} confirmationFailed={params.blad === "potwierdzenie"} />;
}
