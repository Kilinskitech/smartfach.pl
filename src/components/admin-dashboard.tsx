...
const modeLabels = { discover: "Odkryj", launch: "Uruchom", operate: "Prowadź" } as const;
...
const costLabel = (value: number) =>
  value === 0 ? "$0.00" : `$${value < 0.01 ? value.toFixed(6) : value.toFixed(4)}`;
    ...
    <div className="admin-shell">
      ...
      <main className="admin-main">
        ...
        <section className="admin-grid" id="zuzycie">
          <article className="admin-panel admin-usage-panel"><div className="admin-panel-heading"><div><Activity size={20} /><span><small>RZECZYWISTE UŻYCIE</small><h2>Koszt OpenRouter</h2></span></div></div><div className="admin-cost-strip"><span><CircleDollarSign size={18} /><small>KOSZT ŁĄCZNY</small><strong>{costLabel(snapshot.totals.costUsd)}</strong></span><span><small>TOKENY</small><strong>{snapshot.totals.totalTokens.toLocaleString("pl-PL")}</strong></span><span><small>ODPOWIEDZI</small><strong>{snapshot.totals.measuredResponses}</strong></span></div><p className="admin-panel-footnote">Koszt pochodzi z pola usage zwróconego przez OpenRouter dla każdego żądania i jest przypisany do użytkownika.</p></article>
        ...
        <section className="admin-panel admin-users" id="uzytkownicy"><div className="admin-panel-heading"><div><Users size={20} /><span><small>BAZA KLIENTÓW SAAS</small><h2>Użytkownicy i subskrypcje</h2></span></div></div><div className="admin-table-head"><span>Użytkownik</span><span>Plan</span><span>Status</span><span>Karta</span><span>Zużycie</span></div>{snapshot.users.length === 0 ? <div className="admin-quality-empty"><Users size={25} /><h3>Nie ma jeszcze użytkowników</h3><p>Pierwsze prawdziwe konto pojawi się po rejestracji przez Supabase.</p></div> : snapshot.users.map((user) => <Link prefetch={false} className="admin-user-row" href={`/admin/uzytkownicy/${user.id}`} key={user.id}><span className="admin-user-identity"><i>{(user.name || user.email).slice(0,2).toUpperCase()}</i><span><strong>{user.name || user.email}</strong><small>{user.email} · typ {modeLabels[user.accountType]}</small></span></span><strong>{user.plan}</strong><span><Status active={["active","trialing"].includes(user.status)}>{statusLabels[user.status] ?? user.status}</Status></span><span>{user.paymentMethodAttached ? "Podpięta" : "Brak"}</span><span className="admin-user-usage">{user.usedCredits} / {user.allowance}<ArrowRight size={15} /></span></Link>)}</section>
        <section className="admin-privacy" id="bezpieczenstwo"><div><ShieldCheck size={24} /></div><section><p className="eyebrow">JEDEN ADMINISTRATOR</p><h2>Brak zbędnego systemu ról.</h2><p>Dostęp ma wyłącznie konto o UUID zapisanym po stronie serwera. Widok rozmów tworzy wpis audytowy z administratorem, użytkownikiem i czasem otwarcia.</p></section><ul><li><CheckCircle2 size={15} /> użytkownik → rozmowy → wiadomości</li><li><CheckCircle2 size={15} /> koszt OpenRouter per użytkownik</li><li><CheckCircle2 size={15} /> brak lokalnego profilu testowego</li></ul></section>
        <footer className="admin-footer"><span>Odczyt: {new Date(snapshot.generatedAt).toLocaleString("pl-PL")}</span><Link href="/app">Otwórz SmartFach <ArrowRight size={14} /></Link></footer>
      </main>
    </div>
  );
}
