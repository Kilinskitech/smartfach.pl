  ...
  return (
    <div className="admin-shell">
      ...
      <main className="admin-main">
        ...
        <section className="admin-panel admin-users" id="uzytkownicy"><div className="admin-panel-heading"><div><Users size={20} /><span><small>BAZA KLIENTÓW SAAS</small><h2>Użytkownicy i subskrypcje</h2></span></div></div><div className="admin-table-head"><span>Użytkownik</span><span>Plan</span><span>Status</span><span>Karta</span><span>Zużycie</span></div>{snapshot.users.length === 0 ? <div className="admin-quality-empty"><Users size={25} /><h3>Nie ma jeszcze użytkowników</h3><p>Pierwsze prawdziwe konto pojawi się po rejestracji przez Supabase.</p></div> : snapshot.users.map((user) => <Link prefetch={false} className="admin-user-row" href={`/admin/uzytkownicy/${user.id}`} key={user.id}><span className="admin-user-identity"><i>{(user.name || user.email).slice(0,2).toUpperCase()}</i><span><strong>{user.name || user.email}</strong><small>{user.email} · buduje własny przychód</small></span></span><strong>{user.plan}</strong><span><Status active={["active","trialing"].includes(user.status)}>{statusLabels[user.status] ?? user.status}</Status></span><span>{user.paymentMethodAttached ? "Podpięta" : "Brak"}</span><span className="admin-user-usage">{user.usedCredits} / {user.allowance}<ArrowRight size={15} /></span></Link>)}</section>
