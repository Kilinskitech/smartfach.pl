...
export function termsDocument(operator: Operator): LegalDocument {
  return {
    ...
    sections: [
      ...
      { title: "3. Wymagania techniczne i aplikacja na telefon", paragraphs: [
        ...
        "SmartFach można dodać do ekranu początkowego jako aplikację internetową (PWA), jeżeli przeglądarka i system obsługują tę funkcję. Instalacja jest opcjonalna i nie jest osobno płatna; korzysta z tego samego konta i abonamentu. Nie wymaga zakupu w App Store ani Google Play. AI, logowanie, płatności i synchronizacja wymagają internetu. Zakres instalacji zależy od urządzenia; instrukcje są na stronie Pobierz aplikację.",
      ...
      { title: "5. Ceny, próba i płatności cykliczne", paragraphs: [
        `SmartFach Lite kosztuje ${plans.lite.price} miesięcznie, a SmartFach Pro ${plans.pro.price} miesięcznie. Są to całkowite ceny w PLN dla użytkownika, obejmujące podatki należne przy sprzedaży. Abonament jest płatny z góry za każdy miesięczny okres. Nie pobieramy osobnej opłaty za instalację PWA.`,
      ...
      { title: "6. Miesięczny limit i jego zwiększenie", paragraphs: [
        `Lite zawiera ${plans.lite.monthlyCredits} jednostek rozliczeniowych na okres, Pro ${plans.pro.monthlyCredits}. W aplikacji pokazujemy procent wykorzystania limitu. Jednostki służą do rozliczenia pracy AI; nie są walutą, pieniądzem elektronicznym ani tokenami konkretnego modelu. Nie obiecujemy stałej liczby wiadomości.`,
        "Koszt poprawnie obsłużonego żądania, obejmujący przetworzenie kontekstu, odpowiedź i ewentualne narzędzia, przeliczamy z kosztu dostawcy: 0,01 USD odpowiada jednej jednostce, z zaokrągleniem w górę i minimum jednej jednostki za żądanie. Jeżeli dostawca nie zwróci pomiaru, naliczamy jedną jednostkę za wiadomość, dodatkowo dwie za każde zdjęcie oraz dwie za użyte wyszukiwanie internetowe. Wydłużenie rozmowy, obrazy i research mogą więc przyspieszyć wykorzystanie limitu. Wartość USD opisuje przelicznik techniczny, a nie kwotę do wypłaty lub należną od użytkownika.",
        ...
        `Jednorazowe zwiększenia: ${creditPacks.map(pack => `${pack.credits} jednostek za ${pack.price}`).join("; ")}. Cena jest całkowita. Wielkość pakietu, cena i warunki są widoczne przed osobnym zamówieniem. Zakup nie odnawia się automatycznie i nie zmienia abonamentu.`,
...
export function privacyDocument(operator: Operator): LegalDocument {
  return { title: "Polityka prywatności SmartFach", version: legalDocumentVersion, sections: [
    ...
    { title: "11. Cookies i pamięć urządzenia", paragraphs: [
      ...
      "Obecnie w kodzie serwisu nie uruchamiamy marketingowych ani analitycznych narzędzi śledzących, takich jak Meta Pixel lub Google Analytics. Jeśli zostaną dodane, zapewnimy uprzednie informacje i wymagany wybór zgody przed ich uruchomieniem. Zgody marketingowej nie zastępuje akceptacja Regulaminu.",
      "Po instalacji PWA urządzenie zapisuje ikonę i konfigurację aplikacji. Service worker przechowuje wyłącznie publiczny ekran braku internetu; nie zapisuje w swojej pamięci podręcznej rozmów, odpowiedzi API ani stron konta. Sesja logowania nadal korzysta z niezbędnej pamięci przeglądarki. Odinstalowanie nie usuwa danych przechowywanych na koncie i nie anuluje abonamentu.",
    ] },
